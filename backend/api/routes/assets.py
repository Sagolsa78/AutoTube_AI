...
from backend.worker_router import WorkerCapability, dispatch_job
...
async def search_assets(
    query: str, count: int = 10, db: AsyncSession = Depends(get_db)
):
...
async def assign_asset_to_scene(
    ...
    scene = await db.scalar(
        select(Scene).where(Scene.id == scene_id, Scene.user_id == user.id)
    )
    if not scene:
    ...
    return {
        "status": "success",
        "message": "Asset cached and assigned",
        "asset_id": asset.id,
    }
...
async def generate_asset(
    ...
    return {
        "status": "success",
        "job_id": job_id,
        "message": "Asset generation job dispatched",
    }
