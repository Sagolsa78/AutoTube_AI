...
class StoryPlanner:
    ...
    def generate_story(
        ...
        **kwargs,
    ) -> Tuple[StorySpec, str, Dict[str, Any]]:
        """
        Generates a validated StorySpec.
        Returns a tuple of (StorySpec, provider_used, cost_data).
        """
        ...
        for attempt in range(self.max_retries + 1):
            try:
                log.info(
                    f"Generating StorySpec (attempt {attempt + 1}/{self.max_retries + 1})"
                )
                raw_response, provider, cost_data = generate_with_fallback(
                    full_prompt,
                ...
                log.info(
                    f"Successfully generated and validated StorySpec using {provider}"
                )
                return story_spec, provider, cost_data

            except (json.JSONDecodeError, ValueError) as e:
                log.warning(
                    f"Failed to parse or validate StorySpec on attempt {attempt + 1}: {e}"
                )
                last_error = e
        ...
        raise RuntimeError(
            f"Failed to generate valid StorySpec after {self.max_retries + 1} attempts. Last error: {last_error}"
        )
