...
class Evidence(BaseModel):
    ...
    support_score: float = Field(
        default=1.0, description="How strongly this supports the claim (0.0 to 1.0)"
    )
...
class SceneSpec(BaseModel):
    ...
    claim_ids: List[str] = Field(
        default_factory=list, description="IDs of factual claims made in this scene"
    )

    # Visual intent
    visual_intent: str = Field(..., description="High level visual concept")
    stock_query: str = Field(
        default="", description="Specific 3-5 word search query for stock footage"
    )
    generation_prompt: str = Field(
        default="", description="Detailed prompt for AI image/video generation"
    )
    preferred_visual_mode: Literal[
        "STOCK",
        "GENERATED_VIDEO",
        "GENERATED_IMAGE",
        "MOTION_GRAPHIC",
...
class StorySpec(BaseModel):
    ...
    def get_full_text(self) -> str:
        ...
        return " ".join(
            s.narration
            for s in sorted(self.scenes, key=lambda x: x.scene_number)
            if s.narration
        )
