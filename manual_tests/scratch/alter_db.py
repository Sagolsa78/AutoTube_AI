...
try:
    c.execute(
        "ALTER TABLE user_profiles ADD COLUMN default_voice_id VARCHAR DEFAULT 'en-US-ChristopherNeural'"
    )
    c.execute(
        "ALTER TABLE user_profiles ADD COLUMN content_tone VARCHAR DEFAULT 'casual'"
    )
    c.execute("ALTER TABLE user_profiles ADD COLUMN niche_keywords JSON DEFAULT '[]'")
    c.execute(
        "ALTER TABLE user_profiles ADD COLUMN title_style_preference VARCHAR DEFAULT 'curiosity'"
    )
    c.execute(
        'ALTER TABLE user_profiles ADD COLUMN hashtag_set JSON DEFAULT \'["shorts", "viral"]\''
    )
