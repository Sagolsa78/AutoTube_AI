...
from backend.models.models import User, Channel, Idea, Script, Video, VideoStatus
...
async def test_cross_user_isolation_channels_and_videos():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        res_b_get_channel_a = await ac.get(f"/api/channels/{channel_a_id}", headers=headers_b)
        assert res_b_get_channel_a.status_code in (403, 404)

        # User B attempts to update User A's channel — must return 404 or 403
        res_b_patch_channel_a = await ac.patch(f"/api/channels/{channel_a_id}", json={"name": "Hacked"}, headers=headers_b)
        ...
        res_b_preview_fake = await ac.get("/api/videos/fake-video-id/preview", headers=headers_b)
