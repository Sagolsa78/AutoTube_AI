            ...
            def scalars(self):
                class MockScalars:
                    ...
                    def first(self):
                        ...
                        return Script(id="script1", idea_id="idea1", status=ScriptStatus.used_in_render)
                return MockScalars()
        return MockResult()
...
async def test_metadata_failure_no_auto_publish(mock_session_local, mock_gen, mock_upload, mock_assemble, mock_tts, mock_fetch, mock_getsize):
    ...
    channel = Channel(id="ch1", auto_approve=True, title_style_preference="curiosity", niche="science_wow")
...
async def test_pytrends_fails_idea_generation_proceeds(mock_gen, mock_trends):
    ...
    channel = Channel(id="ch1", user_id="default-user", niche="test", niche_keywords=["test"])
