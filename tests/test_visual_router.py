...
async def test_visual_router_auto_mode(mock_router):
    ...
    with patch.object(
        mock_router, "_resolve_stock", new_callable=AsyncMock
    ) as mock_stock, patch.object(
        mock_router, "_resolve_generated_image", new_callable=AsyncMock
    ) as mock_image, patch.object(
        mock_router, "_resolve_generated_video", new_callable=AsyncMock
    ) as mock_video:
...
@pytest.mark.asyncio
@patch("integrations.providers.visuals.comfyui_provider.ComfyUIProvider")
async def test_visual_router_generation_prompt(mock_client_class, mock_router):
    ...
    with patch("engine.visuals.router.AsyncSessionLocal") as mock_db:
        ...
        mock_client.generate_image.assert_called_once_with(
            "A very specific generation prompt", "/tmp/visuals"
        )
